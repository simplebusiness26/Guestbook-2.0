...
export default function Review() {
  ...
  async function pickImages() {
    ...
    if (selectedImages.length >= 3) {
      setErrorMessage("You can add a maximum of 3 photos.");
      return;
    }
    ...
    try {
      ...
      setErrorMessage(
        "The photo picker could not return to the app. Try opening the Replit preview in a separate browser tab."
      );
  ...
  async function uploadReviewImages(userId) {
    ...
    for (
      ...
      const filePath =
        `${userId}/business-reviews/${id}/${fileName}`;
  ...
  async function submitReview() {
    ...
    try {
      ...
      if (selectedImages.length > 0) {
        photoUrls =
          await uploadReviewImages(user.id);
      }
